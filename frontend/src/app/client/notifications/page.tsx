...
export default function ClientNotificationsPage() {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <PageHeader
        title="Owner Provider Notifications"
        description="Alerts on new booking requests, reviews, and platform verification updates."
        actions={
          unreadCount > 0 ? (
            <Button
          ...
          {notifications.map((notif: any) => (
            <Card
              ...
              className={`p-4 flex justify-between items-start gap-4 ${
                notif.is_read ? 'bg-zinc-900/40 border-zinc-800/80 text-zinc-400' : 'bg-zinc-900 border-emerald-500/30 text-zinc-100'
              }`}
            >
