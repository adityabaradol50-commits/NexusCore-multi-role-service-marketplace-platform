...
export default function ClientNotificationsPage() {
  ...
  const markAllReadMutation = useMutation({
    mutationFn: async () => {
      ...
      await Promise.all(unread.map((n: any) => api.patch(`/client/notifications/${n.id}/read`)));
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['clientNotifications'] });
    },
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">Provider Notifications</h1>
          <p className="text-xs text-slate-400 mt-1">Alerts on new booking requests, reviews, and platform verification updates.</p>
        </div>
        ...
        {unreadCount > 0 && (
          <button
            ...
            className="px-3.5 py-2 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-emerald-400 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-colors shrink-0"
          >
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Customer booking requests and administrative notices will appear here in real time."
        />
      ) : (
            ...
            >
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-3 pt-1">
                  ...
                  {notif.link && (
                    <Link
                      href={notif.link.startsWith('/') ? notif.link : '/client/requests'}
